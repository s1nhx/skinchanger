...
void changeskin(int params) {
	...
	if (!xz) { // if ped ISN'T in air
		...
		SAMP::pSAMP->addMessageToChat(-1, "[skinchanger FIXED 1.2] использование команды в воздухе запрещено.");
	}
}
...
void __stdcall GameLoop() {
	...
	if (!initialized) {
		if (SAMP::pSAMP->LoadAPI()) {
			...
			SAMP::pSAMP->addMessageToChat(-1, "[skinchanger FIXED 1.2] loaded. cmd:  /sskin [id]  | vk/@sinhxxx");
